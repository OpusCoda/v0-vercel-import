...
export function marketToCard(entry: MarketWithId): MarketCardProps {
  ...
  // Volume = cumulative traded flow. Market balance = actual PLS the contract
  // holds for this market (seed + net buys − sells − claims). This is the
  // intuitive "money in the market" figure — unlike yesPool + noPool, which is
  // AMM pool depth and barely moves on a buy (one reserve rises as the other
  // falls). remainingMarketBalance tracks real PLS and moves ~1:1 with deposits.
  const volumePls = Number(formatUnits(m.totalVolume, 18))
  const liquidityPls = Number(formatUnits(m.remainingMarketBalance, 18))
  return {
    type: "probability",
